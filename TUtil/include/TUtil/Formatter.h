...
#if CHAR_MIN == 0
#define TUTIL_UNSIGNED_CHAR

#elif CHAR_MIN == SCHAR_MIN
#define TUTIL_SIGNED_CHAR
...
//If char is signed we can provide an operator that prints an byte
		...
		Formatter& operator<<(std::uint8_t value);
#endif

		Formatter& operator<<(std::uint16_t value);
		Formatter& operator<<(std::int16_t value);

		Formatter& operator<<(std::uint32_t value);
		Formatter& operator<<(std::int32_t value);

		Formatter& operator<<(std::uint64_t value);
		Formatter& operator<<(std::int64_t value);
		...
		template<typename T>
		Formatter& Base(T value, std::uint8_t base)
		{
	...
	private:
		...
		template<typename T>
		inline void PrintUnsignedInteger(T value, std::uint8_t base = 10)
		{
		...
		template<typename T>
		inline void PrintSignedInteger(T value, std::uint8_t base = 10)
		{
			if (value < static_cast<T>(0))
			{
				PrintChar('-');
	...
	public:
		...
		template<typename T> inline Formatter& Base(const T& value, std::uint8_t base) { return m_Wrapper.Base(value, base); }
...
#ifndef TUTIL_FORMAT_FILE_BUF_SIZE
#define TUTIL_FORMAT_FILE_BUF_SIZE 512
#endif

	template<typename T>
	static std::size_t MaxDigits(T value, std::uint8_t base = 10)
	{
	...
	public:
		...
		template<typename T> FormatFile& Base(const T& value, std::uint8_t base) { TryFlush(MaxDigits<T>(value, base)); m_Wrapper.Base(value, base); return *this; }
...
#ifdef TUTIL_SIGNED_CHAR
		FormatFile& WriteImpl(std::uint8_t);
#endif
		FormatFile& WriteImpl(std::uint16_t);
		FormatFile& WriteImpl(std::int16_t);
		FormatFile& WriteImpl(std::uint32_t);
		FormatFile& WriteImpl(std::int32_t);
		FormatFile& WriteImpl(std::uint64_t);
		FormatFile& WriteImpl(std::int64_t);

		FormatFile& WriteImpl(float);
		FormatFile& WriteImpl(double);
	...
	private:
		...
		char m_Buf[TUTIL_FORMAT_FILE_BUF_SIZE];
		Formatter m_Wrapper;
		FILE* m_File;
	};
